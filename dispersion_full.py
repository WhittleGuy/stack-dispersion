################################################################################
# Atmospheric Dispersion Model (Gaussian, Total Reflection, [g/m^3] || [curies/m^3])
# Brandon Whittle
# D. B. Turner Model (1967)
################################################################################

from matplotlib import pyplot as plt
import numpy as np
import pandas as pd
import seaborn as sb

############################## CONSTANT TABLES #################################


def atmos():
    # Atmospheric conditions table
    atmos = [
        ", Wind [m/s]",
        "Day, Strong",
        "Day, Moderate",
        "Day, Slight",
        "|, |",
        "Night, Low",
        "Night, Moderate",
    ]
    atmos = pd.DataFrame(
        [
            ["<2", "A", "A-B", "B", "|", "", ""],
            ["2-3", "A-B", "B", "C", "|", "E", "F"],
            ["3-5", "B", "B-C", "C", "|", "D", "E"],
            ["5-6", "C", "C-D", "D", "|", "D", "D"],
            [">6", "C", "D", "D", "|", "D", "D"],
        ],
        columns=atmos,
    )
    a = atmos.columns.str.split(", ", expand=True).values
    atmos.columns = pd.MultiIndex.from_tuples(
        [("", x[0]) if pd.isnull(x[1]) else x for x in a]
    )

    return atmos

# stability: Stability Class [A,B,C,D,E,F]
# var [a,c,d,f]
# dis downwind distance [km]


def classConsts(stability, var, dis):
    consts = [
        [213, 440.8, 1.941, 9.27, 459.7, 2.094, -9.6],
        [156, 100.6, 1.149, 3.3, 108.2, 1.098, 2],
        [104, 61, 0.911, 0, 61, 0.911, 0],
        [68, 33.2, 0.725, -1.7, 44.5, 0.516, -13],
        [50.5, 22.8, 0.678, 1.3, 55.4, 0.305, -34],
        [34, 14.35, 0.74, -0.35, 62.6, 0.18, -48.6],
    ]

    row = ord(stability) - 65  # See ASCII Table: https://www.asciitable.com/

    if var == "a":
        col = 0
    if dis < 1:
        if var == "c":
            col = 1
        if var == "d":
            col = 2
        if var == "f":
            col = 3
    elif dis >= 1:
        if var == "c":
            col = 4
        if var == "d":
            col = 5
        if var == "f":
            col = 6

    return float(consts[row][col])


################################### FUNCTIONS ##################################
# v_s = 0         # Stack velocity                                      [m/s]
# d = 0           # Stack diameter                                      [m]
# mu = 0          # Wind speed                                          [m/s]
# P = 0           # Pressure                                            [kPa]
# T_s = 0         # Stack temperature                                   [K]
# T_a             # Air Temperature                                     [K]
def get_plume_height(h, v_s, d, mu, P, T_s, T_a):
    return h + (v_s * d / mu) * (
        1.5 + (2.68 * 10 ** (-2) * P * ((T_s - T_a) / T_s) * d)
    )


# a = 0           # Table 12-11 (p566)
# x = 0           # Downwind distance                                      [km]
def get_sigma_y(stability, x):
    if x == 0:
        x += 0.0001
    return classConsts(stability, "a", x) * x ** 0.894


# c = 0           # Table 12-11 (p566)
# d = 0           # Table 12-11 (p566)
# f = 0           # Table 12-11 (p566)
# x = 0           # Downwind distance                                      [km]
def get_sigma_z(stability, x):
    if x == 0:
        x += 0.0001
    return classConsts(stability, "c", x) * x ** classConsts(
        stability, "d", x
    ) + classConsts(stability, "f", x)


# Q = 0           # Uniform emission rate of pollutants                    [g/s] || [curies/s]
# mu = 0          # Mean wind speed affecting plume                        [m/s]
# sigma_y = 0     # Horizontal standard deviation of plume concentration   [m]
# sigma_z = 0     # Vertical standard deviation of plume concentration     [m]
# x = 0           # Downwind coordinate                                    [m]
# y = 0           # Lateral coordinate                                     [m]
# z = 0           # Vertical coordinate                                    [m]
# H = 0           # Plume height (sum of actual height and plume rise)     [m]
def turner(x, y, z, H, Q, mu, sigma_y, sigma_z):
    return np.round(
        (
            (Q / (2 * np.pi * sigma_y * sigma_z * mu))
            * np.exp(-0.5 * ((y / sigma_y) ** 2))
            * (
                np.exp(-0.5 * (((z - H) / sigma_z) ** 2))
                + np.exp(-0.5 * (((z + H) / sigma_z) ** 2))
            )
        ),
        10,
    )


def header():
    print("*******************************************************")
    print("Atmospheric Dispersion Model")
    print("Written by Brandon Whittle")
    print("*******************************************************")


def atmosConditions():
    print("-------------------------------------------------------")
    print("                Atmospheric Conditions")
    print("-------------------------------------------------------")
    print(atmos())
    print("-------------------------------------------------------")
    print("*Wind speed is at a height of 10m")
    print("*Day is an estimation of solar radiation")
    print("*Night is an estimation of cloud cover")
    print(
        "*Category 'D' can be assumed for all overcast\n\tconditions, regardless of wind speed"
    )
    print("-------------------------------------------------------")
    return input("Select the appropriate category: ")


##################################### Config ###################################

# Resolutions for distance parameters
H_RES = 10
D_RES = 100
L_RES = 100

###################################### Main ####################################


def main():
    header()

    if int(input("Use static values [0/1]: ")) == 1:
        single = 0
        cat = 'D'  # atmosConditions()  # set manually or set to functino for interactive prompt
        x = 10        # Downwind distance [km]
        y = 1500       # Lateral distance [m]
        z = 20        # Vertical distance [m]
        h = 120.0       # Stack height (physical) [m]
        d = 1.2         # Stack diameter [m]
        P = 95.0        # Ambient air pressure [kPa]
        mu = 4.5        # Ambient wind speed at top of stack [m/s]
        T_a = 298.15    # Ambient temperature [K]
        T_s = 588.15    # Stack temperature [K]
        Q = 1656.2      # Stack emission rate [g/s]/[curies/s]
        v_s = 10.0      # Stack emission velocity [m/s]

    else:
        single = int(input("Run a single calculation [0/1]: "))
        cat = atmosConditions()
        x = float(input("Downwind distance [km]: "))
        y = float(input("Lateral distance [m]: "))
        z = float(input("Vertical distance [m]: "))
        h = float(input("Stack height [m]: "))
        d = float(input("Stack diameter [m]: "))
        P = float(input("Ambient air pressure [kPa]: "))
        mu = float(input("Wind speed [m/s]: "))
        T_a = float(input("Ambient air temperature [K]: "))
        T_s = float(input("Stack temperature [K]: "))
        Q = float(input("Stack emission rate [g/s]: "))
        v_s = float(input("Stack velocity [m/s]: "))

    # Calculate effective plume height
    H = get_plume_height(h, v_s, d, mu, P, T_s, T_a)

    # Return only answer if for single calculation
    if single == 1:
        print(turner(x, y, z, H, Q, mu, get_sigma_y(cat, x), get_sigma_z(cat, x)))

    # Return table of values for various heights if for area
    elif single == 0:

        results = []

        # Adjust ranges to allow zero and be floored ints
        H_RANGE = int(np.floor(z)) + 1
        D_RANGE = int(np.floor(x)) + 1
        L_RANGE = int(np.floor(y))

        # Loop through all height/downwind/lateral values
        # Convert values to get correct array length
        # Convert back (if necessary) when passing to turner function
        for h in range(0, H_RANGE, H_RES):
            results.append([])
            for d in range(0, D_RANGE * 1000 - 900, D_RES):
                results[int(h / H_RES)].append([])
                for l in range(-L_RANGE, L_RANGE + 1, L_RES):
                    results[int(h / H_RES)][int(d / D_RES)].append(
                        turner(
                            (d / 1000),
                            l,
                            h,
                            H,
                            Q,
                            mu,
                            get_sigma_y(cat, d / 1000),
                            get_sigma_z(cat, d / 1000),
                        )
                    )

        # Reshape array
        m, n, r = np.array(results).shape
        out_arr = np.column_stack(
            (np.repeat(np.arange(m), n), np.array(results).reshape(m * n, -1))
        )

        if int(input("Print results (may be clipped) [0/1]: ")) == 1:
            out_df = pd.DataFrame(out_arr)
            print(out_df)

        save = int(input("Save [0/1]: "))
        if save == 1:
            out_df.to_csv("./dispersion.csv")

        if int(input(f"Show heatmap [0/1]: ")):
            plot = int(
                int(input("Height plane to show [multiple of 10; < height boundary]: ")) / H_RES)

            x_ticks = []
            x_tick_labels = []
            for i in range(0, len(results[0][0])+2, 4):
                x_ticks.append((len(results[0][0]))/(len(results[0][0])+1)*i)
                x_tick_labels.append(
                    int(-y + (y*2/(len(results[0][0])+1)*i)))

            y_ticks = []
            y_tick_labels = []
            for i in range(x+1):
                y_ticks.append(i*10)
                y_tick_labels.append(i)

            fig, ax1 = plt.subplots(1, 1, figsize=(10, 10))
            sb.heatmap(results[plot], ax=ax1, square=True,
                       robust=True, cmap="coolwarm")
            ax1.set_yticks(y_ticks)
            ax1.set_yticklabels(y_tick_labels)
            ax1.set_ylabel("Downwind Distance [km]")
            ax1.set_xticks(x_ticks)
            ax1.set_xticklabels(x_tick_labels)
            ax1.set_xlabel("Lateral Distance [m]")
            ax1.set_title(f"Stack Dispersion Heatmap | Height: {plot*10}m")
            plt.show()


if __name__ == "__main__":
    main()
